...
def getAgeAtMeasurementBatchFile(folderPath, files, firstMeasurementFile, selectedSystem):
    ...
    if selectedSystem == "National":
        ...
        #There is for the curent implementation of uEMMARM (v0.2 onwards)
...
def convertToG(accelerationDigital,calibrationFactor, selectedSystem=None):
    """
    ...
    selectedSystem: optional, str
        Inform whether is a specific system, so appropriate measurements can be taken
    ...
    if selectedSystem == "uEMMARM":
        #uEMMARM uses ADXL335, whose zero value is shifted to 1.65V (when measuring 0g outputs 1.65V)
        #THe accelerometer has a sensitivity of +-300 mV/g and a range of +-3g
        #uEMMARM's ADC is also configured at a scale of +/- 4.096V, which leads to 0.125mV/bit
        #In this case, a signal of 1.65V is equal to 13200 bits ([1.65V]/[0.125mV/bit])
        #This shift must be taken from the raw signal when converting it to 'gs'
        convertedValue=(accelerationDigital-13200)*calibrationFactor
    else:
        convertedValue=accelerationDigital*calibrationFactor
